...
public abstract class Sprite implements Movable {
    ...
    public Sprite() {
        ...
        setCenter(new Point(Game_new.R.nextInt(Game_new.DIM.width),
                Game_new.R.nextInt(Game_new.DIM.height)));
    ...
    public void move() {
        ...
        if (center.x > Game_new.DIM.width) {
            setCenter(new Point(1, center.y));
        //left-bounds reached
        } else if (center.x < 0) {
            setCenter(new Point(Game_new.DIM.width - 1, center.y));
        //bottom-bounds reached
        } else if (center.y > Game_new.DIM.height) {
            setCenter(new Point(center.x, 1));
        //top-bounds reached
        } else if (center.y < 0) {
            setCenter(new Point(center.x, Game_new.DIM.height - 1));
        //in-bounds
        } else {
    ...
    protected int somePosNegValue(int seed) {
        int randomNumber = Game_new.R.nextInt(seed);
